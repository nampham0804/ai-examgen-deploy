...
def list_learning_outcomes(course_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    learning_outcomes = learning_outcome_service.list_learning_outcomes(db, course_id, current_user.id)
    return {
...
def create_learning_outcome(
    ...
    learning_outcome = learning_outcome_service.create_learning_outcome(db, course_id, payload, current_user.id)
    return {
...
def update_learning_outcome(
    ...
    learning_outcome = learning_outcome_service.update_learning_outcome(db, learning_outcome_id, payload, current_user.id)
    return {
        "data": LearningOutcomeResponse.model_validate(learning_outcome),
...
def delete_learning_outcome(learning_outcome_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    learning_outcome_service.delete_learning_outcome(db, learning_outcome_id, current_user.id)
    return {
