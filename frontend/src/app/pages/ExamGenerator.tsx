      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {selectedBlueprint && qualityInfo ? (
            ...
            <div className="bg-gray-100 dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              <div className="flex items-center gap-3 mb-3 text-gray-400">
                <CheckCircle className="w-6 h-6" />
                <h3 className="font-semibold">Quality Validation</h3>
              </div>
              <p className="text-sm text-gray-400">Chọn Blueprint để xem chất lượng ma trận</p>
            </div>
          )}
        </div>

        {/* Info Panel Placeholder or Exam Preview */}
        <div className="lg:col-span-2 space-y-6">
          {generatedExamId ? (
            <div className="border border-slate-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm h-full max-h-[85vh] overflow-y-auto">
              <ExamPreview examId={generatedExamId} onSaved={() => {
                setGeneratedExamId(null);
                setStep(examConfig.blueprintId ? 2 : 1);
                setAlertInfo({ type: 'success', message: 'Đề thi đã được lưu thành công! Bạn có thể tiếp tục tạo đề mới.' });
              }} />
            </div>
          ) : (
